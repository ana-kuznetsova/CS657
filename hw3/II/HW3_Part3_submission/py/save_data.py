...
def save_data(DICT, STATS, MAXLEN, TRANSFORM, PART):
    '''
    Function normalizes, standardizes data and apply masks to it.
    Arguments:
        DICT: path to mapping dictionary
        STATS: mean, std, min, max vectors
        MAXLEN: length of the largest speech signal in data set
        TRANSFORM: transformation applied to data (norm, IBM, IRM, etc.)
        PART: frame of the modified data set (train, dev)
    '''    
    train_tups = []
...
def find_maxlen(DICT):
    '''
    Finds maximum length of the signal in the data set
    '''
    print('Reading dict...')
